...
public final class VarIntDataStream {
    ...
    public static void writeString(DataOutput output, String data) {
        writeBytes(output, data.getBytes(StandardCharsets.UTF_8));
    }

    @SneakyThrows
    public static void writeBytes(DataOutput output, byte[] data) {
        writeUnsignedVarInt(output, data.length);
        output.write(data);
    }

    @SneakyThrows
    public static byte[] readBytes(DataInput input) {
        int len = (int) readUnsignedVarInt(input);
